...
class CassaNiquel:
    ...
    def __init__(self,level=1,balance = 0):
        ...
        self.balance = balance
        self.initial_balnce = self.balance

    def _gen_permutations(self):
        permutations = list(itertools.product(self.SIMBULOS.keys(), repeat=3))
        for j in range(self.level):
            for i in self.SIMBULOS.keys():
    ...
    def _emojize(self, emojis):
        return ''.join(tuple(chr(int(self.SIMBULOS[code], 16)) for code in emojis))

    def _check_result_user(self, result):
        x = [result[0] == x for x in result]
        return True if all(x) else False

    def _upadete_balance(self, amount_bet, result, player: Player):
        if self._check_result_user(result):
    ...
    def play(self, amount_bet, player: Player):
        ...
        self._upadete_balance(amount_bet, result, player)
    @property
    def gain(self):
        return self.initial_balnce + self.balance

maquina = CassaNiquel(level=3)
...
# parte onde mostras os graficos de perca e ganhos :
